...
use warg_protocol::registry::{
    Checkpoint as WargCheckpoint, LogId, LogLeaf, MapLeaf, RecordId, RegistryLen,
    TIMESTAMPED_CHECKPOINT_SIGNING_PREFIX,
};
...
impl ComputeCheckpoint for Component {
    fn signing_prefix() -> Vec<u8> {
        TIMESTAMPED_CHECKPOINT_SIGNING_PREFIX.to_vec()
    }

    fn checkpoint_id(checkpoint: Checkpoint) -> String {
        let warg_checkpoint = WargCheckpoint {
            log_root: AnyHash::from_str(&checkpoint.log_root).unwrap().into(),
            map_root: AnyHash::from_str(&checkpoint.map_root).unwrap().into(),
            log_length: checkpoint.log_length as RegistryLen,
        };

        let checkpoint_id: AnyHash = Hash::<Sha256>::of(warg_checkpoint).into();
