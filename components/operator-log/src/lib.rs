...
use bindings::warg::operator_log::types::{Hash, Timestamp};
...
use warg_protocol::registry::{LogId as WargLogId, RecordId as WargRecordId};
...
impl OperatorRecords for Component {
    ...
    fn append(envelope: Envelope) -> Result<RecordId, OperatorValidationError> {
        let signature = match Signature::from_str(&envelope.signature) {
            ...
            Err(err) => {
                return Err(OperatorValidationError::SignatureParseFailure(
                    err.to_string(),
                ))
            }
        };
    ...
    fn encode(rec: OperatorRecord) -> Result<EncodedOperatorRecord, OperatorEncodeErrno> {
